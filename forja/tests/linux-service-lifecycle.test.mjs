...
async function waitUntil(predicate, child, label) {
  const until = Date.now() + 10_000;
  while (Date.now() < until) {
    assert.equal(child.exitCode, null, `worker exited while waiting for ${label}`);
    if (await predicate()) return;
    await new Promise((resolve) => setTimeout(resolve, 40));
  }
  throw new Error(`worker did not reach ${label} within 10s`);
}
...
test('real read-only queue serves, drains on SIGTERM and can start again without a stale lock',
  {skip: !supported}, async (t) => {
    const home = await mkdtemp(join(tmpdir(), 'forja-live-worker-'));
    const state = join(home, 'state');
    await import('node:fs/promises').then(({mkdir}) => mkdir(state, {mode:0o700}));
    const children = [];
    t.after(async () => {
    ...
    for (let round = 0; round < 2; round++) {
      ...
      await waitUntil(async () => !await absent(join(state, 'jobs')) &&
        await absent(join(state, 'worker.lock')), running.child, 'idle polling');
