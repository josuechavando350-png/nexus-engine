...
// Dedicated CI mounts a real, private 1 MiB tmpfs at this path. All FORJA
// operations run unprivileged and observe the kernel's actual ENOSPC error.
...
test('real full filesystem rejects snapshot without publishing partial state',
  { skip: !volume }, async (t) => {
    requireVolume();
...
test('real full filesystem rejects restore without exposing incomplete destination',
  { skip: !volume }, async (t) => {
    requireVolume();
    const f = await fixture(t);
