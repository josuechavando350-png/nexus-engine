...
- Las pruebas de #69 se ejecutan solo cuando el runtime expone `crypto.encapsulate` y `crypto.decapsulate`. En Node 22 el resto de la suite sigue verificándose sin inventar una ruta insegura.
- Nuevo script `npm run verify:mlkem` que genera `evidence/mlkem-69-verification.json` con resultado PASS o SKIPPED_UNSUPPORTED_NODE según la plataforma.
- `package.json` ahora declara compatibilidad general con Node >=22.16.0; el requisito de Node 24 queda acotado al motor #69.
