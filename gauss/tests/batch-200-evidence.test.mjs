...
test('first 200 preserved and 3 distinct new operators run in connected GAUSS/Quantum/WALLE proof',async()=>{
 assert.equal(GAUSS_IMPLEMENTED_LAYERS.length,203);
 assert.equal(problem.tasks.length,203);
 assert.equal(new Set(problem.tasks.map(x=>x.layerId)).size,203);
 assert.equal(new Set(problem.tasks.map(x=>x.taskId)).size,203);
 assert.equal(new Set(GAUSS_IMPLEMENTED_LAYERS.map(x=>x.execute)).size,203);
 ...
 assert.equal(problem.tasks.at(-3).layerId,'GAUSS.MATH.CRT_GENERAL.059');
 assert.equal(problem.tasks.at(-2).layerId,'GAUSS.MATH.FINITE_FIELD_MATRIX_INVERSE.060');
 assert.equal(problem.tasks.at(-1).layerId,'GAUSS.MATH.INTEGER_POLYNOMIAL_RESULTANT.061');
 assert.equal(report.status,'PASS');assert.equal(report.executedLayerCount,203);assert.equal(report.failedLayerCount,0);
 ...
 const verified=await verifyGaussFoundationEvidence({problem,report});assert.equal(verified.executedLayerCount,203);
});
...
test('WALLE independent arithmetic rejects altered extensions even with recomputed output and report hashes',async()=>{
 assert.equal(report.status,'PASS');
 for(const id of ['GAUSS.MATH.CRT_GENERAL.059','GAUSS.MATH.FINITE_FIELD_MATRIX_INVERSE.060','GAUSS.MATH.INTEGER_POLYNOMIAL_RESULTANT.061']){
  ...
  else result.output.resultant='6';
  result.outputSha256=sha256Canonical(result.output);
  const {reportSha256:previous,...unsigned}=fake;void previous;
  fake.reportSha256=sha256Canonical(unsigned);
