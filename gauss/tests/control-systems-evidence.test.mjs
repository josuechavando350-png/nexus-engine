...
const ids = [
 'GAUSS.CONTROL.STATE_SPACE_TRAJECTORY.003',
 'GAUSS.CONTROL.CONTROLLABILITY_GRAMIAN.004',
 'GAUSS.CONTROL.OBSERVABILITY_GRAMIAN.005',
 'GAUSS.CONTROL.EXACT_CONTROLLABILITY_RANK.006',
 'GAUSS.CONTROL.EXACT_OBSERVABILITY_RANK.007',
 'GAUSS.CONTROL.SCALAR_KALMAN_FILTER.008',
 'GAUSS.CONTROL.SCALAR_RTS_SMOOTHER.009',
 'GAUSS.CONTROL.BOUNDED_PID.010',
 'GAUSS.CONTROL.DIAGONAL_LQR.011',
 'GAUSS.CONTROL.SCALAR_LYAPUNOV.012',
 'GAUSS.CONTROL.MIN_ENERGY_REACHABILITY.013',
 'GAUSS.CONTROL.INTERVAL_REACHABILITY.014',
];

test('every distinct control and dynamics operator has exactly-once WALLE coverage', async () => {
...
test('WALLE rejects each forged control output even after all affected hashes are recomputed', async () => {
 for (const id of ids) {
  const fake = structuredClone(report);
  const result = fake.taskResults.find(row => row.layerId === id);
  assert(result, `missing control operator ${id}`);
  result.output = { ...result.output, forgedControlClaim: 1000 };
