...
test("registry exposes exactly 40 executable foundation operators toward an 800-operator target", () => {
  const summary = gaussRegistrySummary();
  assert.equal(summary.targetLayerCount, 800);
  assert.equal(summary.implementedLayerCount, 40);
  assert.equal(GAUSS_IMPLEMENTED_LAYERS.length, 40);
  assert.equal(new Set(GAUSS_IMPLEMENTED_LAYERS.map((row) => row.id)).size, 40);
...
test("end-to-end NEXUS -> GAUSS -> Quantum run executes all 40 implemented operators", async () => {
  ...
  assert.equal(first.executedLayerCount, 40);
