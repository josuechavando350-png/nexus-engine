...
test('AXIOMA reports exactly 851 unique reference-checked operators, not the other 149', () => {
  ...
  assert.equal(report.coveredOperators, 851);
  assert.equal(report.untestedOperators, 149);
  assert.equal(report.validCases, 85100);
  assert.equal(report.passedValidCases, 85100, JSON.stringify(report.suites.flatMap(s => s.failures)));
  assert.equal(report.invalidCases, 2553);
  assert.equal(report.passedInvalidRejections, 2553, JSON.stringify(report.suites.flatMap(s=>s.failures)));
  assert.equal(report.failedValidCases, 0);
  assert.equal(report.failedInvalidRejections, 0);
  assert.equal(report.validPassRate, 1);
  assert.equal(report.coverageRate, 0.851);
  assert.notEqual(report.coverageRate, report.validPassRate);
  assert.equal(report.suites.length,41);
