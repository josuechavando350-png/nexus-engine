...
const defs=[
 ...
 {id:'GAUSS.INFO.BAYES_POSTERIOR.025',make:(i,r)=>{const n=1+r(7);return {prior:pmf(r,n),likelihood:range(n).map(()=>(1+r(16))/16)};},reference:x=>{const masses=x.prior.map((v,i)=>v*x.likelihood[i]),evidence=sum(masses);return {evidence,posterior:masses.map(m=>m/evidence)};},invalid:x=>bad(x,'prior'),verify},
