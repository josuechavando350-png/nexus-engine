/* Independent quotient-language semantics and analytic bounded numerical errors. */
...
function automataVerify(tag,input,actual,expected){
  ...
  // The independently enumerated accepted languages determine the equivalence
  // classes. Class IDs have no mathematical significance; every transition,
  // initial state and accepting class must nevertheless agree with that partition.
  assert.ok(Array.isArray(actual.classes)&&Array.isArray(actual.dfa?.transitions));
  const normalize=classes=>classes.map(group=>group.slice().sort((a,b)=>a-b).join(',')).sort();
  assert.deepStrictEqual(normalize(actual.classes),normalize(expected.classes),'wrong Myhill-Nerode partition');
  assert.deepStrictEqual(actual.classes.flat().slice().sort((a,b)=>a-b),expected.classes.flat().slice().sort((a,b)=>a-b),'missing or duplicated states');
  const accepting=new Set(input.accepting);
  const byState=new Map(actual.classes.flatMap((group,i)=>group.map(state=>[state,i])));
  const dfa={alphabet:input.alphabet,start:byState.get(input.start),accepting:actual.classes.flatMap((group,i)=>accepting.has(group[0])?[i]:[]),transitions:actual.classes.map(group=>input.transitions[group[0]].map(state=>byState.get(state)))};
  assert.deepStrictEqual(actual.dfa,dfa,'invalid quotient transition, start, or final-state semantics');
}
function numericalVerify(tag,input,actual,expected){
  ...
  if(tag.startsWith('QUAD_')){
    // For f(x)=c0+c1*x+c2*x^2 on n uniform intervals, the composite
    // trapezoid error is c2*(b-a)^3/(6*n^2); midpoint is its negative half.
    ...
    if(tag==='QUAD_TRAPEZOID'||tag==='QUAD_MIDPOINT'){
      ...
      assert.ok(Math.abs(actual.integral-expected.integral)<=Math.abs(correction)+1e-11,'quadrature error exceeds polynomial bound');
  ...
  if(tag.startsWith('ODE_')){
    ...
    actual.times.forEach((time,i)=>{
      ...
      // Euler's left-endpoint integral for y'=b*t+c differs by -b*i*dt^2/2.
