...
const defs=[
 ...
 {id:'GAUSS.MATH.BARYCENTRIC_INTERPOLATION.040',make:(i,r)=>{const n=3+r(3),A=r(7)-3,B=r(7)-3,C=r(7)-3,nodes:never=undefined;return {nodes:range(n).map(j=>j-2),values:range(n).map(j=>A+B*(j-2)+C*(j-2)**2),at:n+r(3)};},reference:x=>{const y=x.values,n=x.nodes.length,a=x.nodes[0],d=x.nodes[1]-a,A=y[0],C=(y[2]-2*y[1]+y[0])/(2*d*d),B=(y[1]-y[0]-C*((a+d)**2-a*a))/d;return {value:A+B*(x.at-a)+C*(x.at*x.at-a*a)};}},
