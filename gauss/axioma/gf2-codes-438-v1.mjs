...
const bits=(v,n)=>seq(n).map(i=>v>>i&1),mask=a=>a.reduce((s,v,i)=>s|(v<<i),0),weight=a=>a.reduce((s,v)=>s+v,0);
...
const invert=a=>{const n=a.length;if(rank(a)!==n)return {invertible:false,inverse:null};const b=seq(n).map(i=>a[i].concat(bits(1<<i,n))),r=rref(b);return {invertible:true,inverse:r.rows.map(row=>row.slice(n))};};
const identity=n=>seq(n).map(i=>bits(1<<i,n));
...
function input(tag,i,r){const n=1+i%3,w=1+(i+1)%4,A=seq(n).map(()=>seq(w).map(()=>r(2))),sq=seq(n).map(()=>seq(n).map(()=>r(2))),generator=seq(1+i%3).map(()=>seq(3+i%2).map(()=>r(2))),word=seq(w).map(()=>r(2));
 ...
 if(tag==='GF2_MATMUL'){const cols=1+r(3);return {left:A,right:seq(w).map(()=>seq(cols).map(()=>r(2)))};}
...
function ref(tag,x){const a=x.matrix??x.generator??x.parityCheck??(tag==='GF2_MATMUL'?x.left:null),b=x.right,rr=a?rref(a):null;
 switch(tag){
 ...
 case 'GF2_SOLVE':{const rows=seq(2**a[0].length).map(j=>bits(j,a[0].length)).filter(v=>code(trans(a),v).every((z,k)=>z===x.rhs[k]));const found=rows[0],e=rref(a.map((row,i)=>row.concat(x.rhs[i]))).rows,p=rr.pivots,solution=Array(a[0].length).fill(0);if(found)p.forEach((j,i)=>solution[j]=e[i][a[0].length]);return {consistent:!!found,solution:found?solution:null,unique:!!found&&rr.rank===a[0].length};}
 case 'GF2_MATMUL':return {matrix:mul(a,b)};
 case 'GF2_MATPOWER':{let z=identity(a.length);for(let j=0;j<x.exponent;j++)z=mul(z,a);return {matrix:z};}
 ...
 case 'HAMMING74_ENCODE':{const z=Array(7).fill(0);[2,4,5,6].forEach((p,i)=>z[p]=x.data[i]);for(const p of [1,2,4])z[p-1]=seq(7).filter(j=>Boolean((j+1)&p)&&j!==p-1).reduce((s,j)=>s^z[j],0);return {codeword:z};}
 case 'HAMMING74_DECODE':{const w=x.received.slice(),s=[1,2,4].reduce((t,p)=>t+p*seq(7).filter(j=>Boolean((j+1)&p)).reduce((v,j)=>v^w[j],0),0);if(s)w[s-1]^=1;return {syndrome:s,correctedPosition:s||null,codeword:w,data:[w[2],w[4],w[5],w[6]],assumption:'at most one bit error'};}
