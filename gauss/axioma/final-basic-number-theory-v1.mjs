...
const definitions=tags.map((tag,index)=>({id:`GAUSS.MATH.${tag}.${String(10+index).padStart(3,'0')}`,make:(i,r)=>sample(tag,i,r),reference:x=>ref(tag,x),invalid:x=>{const key=Object.keys(x)[0];return [null,{...x,[key]:null},{...x,[key]:'AXIOMA_NOT_A_NUMBER'}];},...(tag==='EXTENDED_EUCLID'?{verify:(actual,expected,input)=>{assert.equal(actual.gcd,expected.gcd);assert.ok(Number.isSafeInteger(actual.bezoutX)&&Number.isSafeInteger(actual.bezoutY));assert.equal(BigInt(input.a)*BigInt(actual.bezoutX)+BigInt(input.b)*BigInt(actual.bezoutY),BigInt(expected.gcd));}}:{})}));
