...
const rect=r=>{const x=r(31)-15,y=r(31)-15,w=2+2*r(6),h=w+1+r(6);return [[x,y],[x+w,y],[x+w,y+h],[x,y+h]];};
...
const defs=[
 ...
 {id:'GAUSS.MATH.SEGMENT_INTERSECTION.023',make:(i,r)=>{const p=rect(r),[a,b,c,d]=p,m=(a[0]+b[0])/2;switch(i%4){case 0:return {a,b,c:[m,a[1]-2],d:[m,a[1]+2]};case 1:return {a,b,c:[a[0],a[1]+2],d:[b[0],b[1]+2]};case 2:return {a,b,c:a,d:[a[0],a[1]+2]};default:return {a,b,c:[m,a[1]],d:[b[0]+2,a[1]]};}},reference:(x)=>({relation:['PROPER_CROSS','DISJOINT','TOUCH','OVERLAP'][x.a[0]===x.c[0]&&x.c[1]===x.a[1]?2:x.c[1]===x.a[1]&&x.d[1]===x.a[1]?3:x.c[1]===x.a[1]+2?1:0]})},
 {id:'GAUSS.MATH.CONVEX_HULL_2D.024',make:(i,r)=>{const p=rect(r),{x,y,w,h}=xy(p);return {vertices:[p[2],p[0],[x+w/2,y+1],p[3],p[1],p[0]]};},reference:x=>{const p=x.vertices,[x0,y0]=p[1],w=p[0][0]-x0,h=p[0][1]-y0;return {hull:[[x0,y0],[x0+w,y0],[x0+w,y0+h],[x0,y0+h]],count:4};}},
