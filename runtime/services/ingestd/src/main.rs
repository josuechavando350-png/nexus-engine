...
use nexus_ingest::{
    EventHandler, HandlerOutcome, IngestConfig, InMemoryBus, IngestPipeline, MessageBus,
    OutboundMessage,
};
...
impl EventHandler for NormalizeHandler {
    ...
    fn handle(&self, envelope: &EventEnvelope) -> Result<HandlerOutcome> {
        if envelope.stream.starts_with("control.") {
            // Ingest never acts on control traffic. Recognised and dropped.
        ...
        let (record, outcome, mutations) =
            pipeline_for_telemetry(envelope, &candidates, None)?;
...
fn run(logger: &Logger) -> Result<()> {
    let config = match IngestConfig::from_env() {
        Ok(config) => config,
        Err(error) => {
            // A misconfigured broker is a hard failure, not a fallback to a
            // guessed localhost address.
            logger.error(
    ...
    // The in-memory bus is the default transport. `--features kafka` swaps in
    // the Redpanda/Kafka adapter; see nexus_ingest::kafka.
    ...
    // Single-shot drain. A long-running deployment loops here and installs a
    // signal handler that calls `request_shutdown`; that loop lives in the
    // kafka feature path where there is something to poll.
