...
use nexus_event::{topics, NexusError, Result, Timestamp, TraceId};
use nexus_ingest::{InMemoryBus, MessageBus, OutboundMessage};
...
fn main() {
    ...
    let logger = Logger::stderr("orchestratord", level);
...
fn execution_mode_from_env() -> Result<ExecutionMode> {
    match std::env::var("NEXUS_EXECUTION_MODE")
        ...
        .as_str()
    {
        ...
        "physical_non_weaponized" => Ok(ExecutionMode::PhysicalNonWeaponized),
        other => Err(NexusError::invalid(format!(
...
fn run(logger: &Logger) -> Result<()> {
    ...
    let signing_key = std::env::var("NEXUS_SIGNING_KEY")
        .map_err(|_| NexusError::invalid("NEXUS_SIGNING_KEY is required"))?;
    let signer_id =
        std::env::var("NEXUS_SIGNER_ID").unwrap_or_else(|_| "orchestratord".to_string());
    let signer = DevSigner::new(signer_id.clone(), signing_key.as_bytes())?;

    health.set("policy", ComponentState::Up, "industrial baseline");
    ...
    // A deployment replaces this with a consumer on nexus.task.proposals.
    ...
    let world_state = WorldState {
        ...
        known_waypoints: vec![("press-4-front".into(), Waypoint::new(10.0, 0.0, 0.0)?)],
        obstacles: vec![],
    ...
    .with_waypoint("press-4-front", Waypoint::new(10.0, 0.0, 0.0)?)
    .with_reading("probe-a", 96.5);
    ...
    metrics
        .histogram(names::TASK_PROPOSAL_LATENCY_MS)
        .observe(Timestamp::now().delta_millis(started) as f64);
    ...
    match &outcome {
        DispatchOutcome::Dispatch { task, .. } => {
            bus.produce(OutboundMessage::json(
                ...
                task.task_id.as_str(),
                &task.to_json().to_canonical_string(),
            ))?;
            logger.with_trace(&trace).info(
                "task dispatched",
                vec![
                    ("task_id", Value::string(task.task_id.as_str())),
                    ("command", Value::string(task.command.name())),
                ],
            );
        }
        DispatchOutcome::Rejected { code, detail, .. } => {
            metrics.counter(names::POLICY_DENIED).incr();
            logger.with_trace(&trace).warn(
                "proposal rejected",
                vec![
        ...
        DispatchOutcome::AwaitingApproval { request, .. } => {
            metrics.counter(names::POLICY_APPROVAL_REQUIRED).incr();
            logger.with_trace(&trace).info(
                "awaiting human approval",
                vec![("reason", Value::string(&request.reason))],
            );
        }
        DispatchOutcome::SimulationFailed { report, .. } => {
            logger.with_trace(&trace).warn(
                ...
                vec![(
                    "error",
                    Value::string(report.first_error_code().unwrap_or("unknown")),
                )],
            );
        }
    }
    ...
    audit
        .verify_chain()
        .map_err(|error| NexusError::integrity(error))?;
