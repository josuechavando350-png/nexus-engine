...
class Batch209214Tests(unittest.TestCase):
    ...
    def test_conflicting_duplicate_search_observation_fails_closed(self):
        records = [
        ...
        result = run_m209(records, {"m209_min_total_impressions": 1})
        self.assertEqual(result["execution_status"], "ERROR")
        self.assertEqual(result["reason_code"], "DUPLICATE_SEARCH_OBSERVATION_CONFLICT")

    def test_m214_requires_real_query_sample(self):
        result = run_m214(
            [
                {"query": "uno", "page_url": "/a", "clicks": 1, "impressions": 100, "average_position_milli": 10_000},
                {"query": "dos", "page_url": "/a", "clicks": 2, "impressions": 100, "average_position_milli": 10_000},
            ],
            {"m214_min_query_impressions": 100, "m214_min_queries": 3},
        )
        self.assertEqual(result["execution_status"], "INSUFFICIENT_DATA")
        self.assertEqual(result["reason_code"], "INSUFFICIENT_QUERY_CTR_SAMPLE")
