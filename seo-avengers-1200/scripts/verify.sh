...
python3 -m py_compile runtime/__init__.py runtime/catalog.py runtime/seo_avengers_1200.py runtime/batch_201_502.py runtime/batch_203_504.py runtime/batch_205_506.py runtime/batch_207_508.py runtime/batch_601_802.py runtime/wire.py runtime/legacy_bridge.py runtime/service.py tests/test_runtime.py tests/test_batch_201_502.py tests/test_batch_203_504.py tests/test_batch_205_506.py tests/test_batch_207_508.py tests/test_batch_601_802.py tests/test_gateway_edges.py tests/test_outbox_worker.py tests/test_legacy_bridge.py scripts/run.py scripts/process-outbox.py
...
assert len(PRE_GATE_MODULES) == 42
assert all(not row["executable_here"] for row in registry.values() if row["status"] == "RESERVED")
assert sum(1 for row in registry.values() if row["status"] == "IMPLEMENTED_PRODUCTION") == 44
print("1200 contiguous slots; exactly 44 reviewed extended handlers; reserved slots remain non-executable")
PY
