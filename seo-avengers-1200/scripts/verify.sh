...
expected = frozenset({
    ...
    "M301", "M302", "M303", "M304", "M305", "M306", "M307", "M308", "M309", "M310", "M311", "M312",
...
assert len(PRE_GATE_MODULES) == 68
assert all(not row["executable_here"] for row in registry.values() if row["status"] == "RESERVED")
assert sum(1 for row in registry.values() if row["status"] == "IMPLEMENTED_PRODUCTION") == 70
print("1200 contiguous slots; exactly 70 reviewed extended handlers; reserved slots remain non-executable")
PY
