...
def run_m314(records: Any, config: Dict[str, Any]) -> Dict[str, Any]:
    """Measure the search-volume-weighted median competitor pressure across observed site-absent gaps."""
    module = "M314"
    algorithm = "competitive_gap_weighted_median_pressure_monitor"
    raw_hash, dataset, invalid_count, conflicts = _prepare_competitor_records(records)
    early = _competitive_input_error(module, algorithm, records, raw_hash)
    if early is not None:
        return early

    min_gap_volume = normalize_integer(config.get("m314_min_gap_search_volume", 100), 100_000_000_000)
    max_median_count = normalize_integer(
        config.get("m314_max_weighted_median_gap_competitor_count", 3), 100_000
    )
    cfg_hash = canonical_hash({
        "min_gap_search_volume": min_gap_volume if min_gap_volume is not None else "INVALID",
        "max_weighted_median_gap_competitor_count": (
            max_median_count if max_median_count is not None else "INVALID"
        ),
        "population": "site_absent_and_competitor_ranked_count_gt_zero",
        "weight": "search_volume",
        "statistic": "lower_threshold_crossing_weighted_median_competitor_count",
        "median_target": "ceil(total_gap_search_volume/2)",
        "ordering": "competitor_ranked_count_then_keyword_ascending",
    })
    norm_hash = _competitive_norm_hash(dataset, invalid_count, conflicts)
    if min_gap_volume is None or min_gap_volume < 1 or max_median_count is None:
        return compile_receipt(module, algorithm, 1, 1, raw_hash, norm_hash, cfg_hash,
                               "ERROR", "NOT_APPLICABLE", "INVALID_MODULE_CONFIG", {})
    data_quality = _competitive_data_quality_receipt(
        module, algorithm, raw_hash, norm_hash, cfg_hash, conflicts, invalid_count
    ...
    for item in ordered:
        ...
        if cumulative_volume >= median_target:
            median_row = item
            break

    if median_row is None:
        return compile_receipt(module, algorithm, 1, 1, raw_hash, norm_hash, cfg_hash,
                               "ERROR", "NOT_APPLICABLE", "WEIGHTED_MEDIAN_UNRESOLVED", {})

    median_count = median_row["competitor_ranked_count"]
    high = median_count > max_median_count
    return compile_receipt(
        module, algorithm, 1, 1, raw_hash, norm_hash, cfg_hash,
        "SUCCESS", "FINDING" if high else "NO_FINDING",
        "COMPETITIVE_GAP_WEIGHTED_MEDIAN_PRESSURE_HIGH" if high else "COMPETITIVE_GAP_WEIGHTED_MEDIAN_PRESSURE_WITHIN_POLICY",
        {"gap_search_volume": gap_volume,
         "gap_keyword_count": len(gap_rows),
         "weighted_median_target_search_volume": median_target,
         "weighted_median_gap_competitor_count": median_count,
         "weighted_median_support_keyword": median_row["keyword"],
         "weighted_median_support_search_volume": median_row["search_volume"],
         "invalid_records_count": invalid_count},
    )
