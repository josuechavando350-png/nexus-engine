...
export default function HomePage() {
  return <main id="main-content">
    <section className="hero" aria-labelledby="hero-title">
      <Image src="/hero-editorial.webp" alt="Interior arquitectónico nocturno con vista a la ciudad" fill priority sizes="100vw" />
    ...
    <section className="products"><div className="products-heading"><p className="brand">NEXUS</p><h2>Una tecnología.<br/>Cuatro sistemas.</h2><p>Inteligencia, experiencia, posicionamiento y cómputo avanzado forman parte de una misma visión.</p></div><div className="product-grid">{products.map(([name,label,copy,tags])=><article key={name}><p className="brand">NEXUS</p><h3>{name}</h3><span>{label}</span><div className="sigil" aria-hidden="true"><i/><i/></div><p>{copy}</p><small>{tags}</small><a href="#contact">Saber más <i /></a></article>)}</div></section>
