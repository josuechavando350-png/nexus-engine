  ...
  return <PageShell>
    <section className="cp-sat-hero">
      <div className="cp-wrap cp-sat-hero-grid">
        ...
        <div className="cp-sat-document" aria-label="Ilustración editorial de una comunicación por revisar"><div className="cp-sat-document-top"><span>DOCUMENTO / REFERENCIA</span></div><span className="cp-sat-document-rule"/><strong>Una comunicación no cuenta toda la historia.</strong><p>El tipo de aviso, el periodo y la autoridad cambian el análisis.</p><span className="cp-sat-document-end">CANO · ANÁLISIS PENAL FISCAL</span></div>
      </div>
    </section>
    <section className="cp-sat-questions"><div className="cp-wrap"><div className="cp-sat-questions-heading"><span>LECTURA DEL CONTEXTO</span><h2>Qué conviene ubicar <em>primero.</em></h2></div><div className="cp-sat-question-list">{signals.map(([title,copy])=><article key={title}><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
