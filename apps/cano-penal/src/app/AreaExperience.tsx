...
export function AreaExperience({ slug, heading, paragraphs: p }: Props) {
  switch (slug) {
      ...
      return <>
        <section className="cp-af-hero">
          <div className="cp-wrap cp-af-hero-grid">
            <div><p className="cp-area-overline">CANO / Defensa especializada / 01</p><h1>{heading}</h1><p className="cp-af-intro">{p[0]}</p></div>
    ...
    case "delitos-patrimoniales-y-fraude":
      return <>
        <section className="cp-ap-hero"><div className="cp-wrap cp-ap-hero-grid"><div><p className="cp-area-overline">02 / Patrimonio y prueba</p><h1>{heading}</h1></div><div className="cp-ap-hero-panel"><span>El punto de partida</span><p>{p[0]}</p></div></div></section>
        <section className="cp-ap-analysis"><div className="cp-wrap"><header><p className="cp-area-overline">Lectura del asunto</p><h2>Primero, distinguir los hechos.</h2></header><div className="cp-ap-analysis-grid"><article><span>01 — Naturaleza de los hechos</span><p>{p[1]}</p></article><article><span>02 — Alcance de la representación</span><p>{p[2]}</p></article></div></div></section>
    ...
    case "homicidio-y-delitos-violentos":
      return <>
        <section className="cp-ah-hero"><img src="/media/audiencia-03.jpg" alt="" aria-hidden="true" /><div className="cp-ah-shade" /><div className="cp-wrap cp-ah-hero-inner"><p className="cp-area-overline">03 / Atención penal</p><h1>{heading}</h1><p>{p[0]}</p><ContactLink>Contactar al abogado</ContactLink></div></section>
    ...
    case "delitos-sexuales":
      return <>
        <section className="cp-as-hero"><div className="cp-wrap cp-as-hero-inner"><p className="cp-area-overline">04 / Discreción y rigor</p><span className="cp-as-watermark" aria-hidden="true">RESERVA</span><h1>{heading}</h1><p>{p[0]}</p></div></section>
    ...
    case "corrupcion-y-administracion-publica":
      return <>
        <section className="cp-ac-hero"><div className="cp-wrap"><div className="cp-ac-topline"><span>CANO / Materia penal</span><span>05 / Servicio público</span></div><h1>{heading}</h1><div className="cp-ac-hero-foot"><span>Administración pública <i aria-hidden="true">/</i> Particulares</span><p>{p[0]}</p></div></div></section>
    ...
    case "despojo-y-defensa-de-victimas":
      return <>
        <section className="cp-av-hero"><div className="cp-av-image" role="presentation" /><div className="cp-wrap cp-av-hero-inner"><p className="cp-area-overline">06 / Representación de víctimas</p><h1>{heading}</h1><p>{p[0]}</p></div></section>
    ...
    case "justicia-penal-para-adolescentes":
      return <>
        <section className="cp-aj-hero"><div className="cp-wrap cp-aj-hero-grid"><div><p className="cp-area-overline">07 / Sistema especializado</p><h1>{heading}</h1></div><aside><span aria-hidden="true">07</span><p>{p[0]}</p></aside></div></section>
        <section className="cp-aj-process"><div className="cp-wrap"><div className="cp-aj-process-title"><span>Un procedimiento propio</span><h2>Comprender el sistema antes de decidir.</h2></div><div className="cp-aj-process-columns"><article><span>01 / Decisiones iniciales</span><p>{p[1]}</p></article><article><span>02 / Acompañamiento</span><p>{p[2]}</p></article></div></div></section>
    ...
    case "amparo-recursos-y-apelaciones":
      return <>
        <section className="cp-aa-hero"><div className="cp-wrap cp-aa-hero-grid"><div><p className="cp-area-overline">08 / Revisión de resoluciones</p><h1>{heading}</h1><p>{p[0]}</p></div><aside><span aria-hidden="true">↗</span><p>Revisar la resolución y el momento procesal.</p></aside></div></section>
        <section className="cp-aa-path"><div className="cp-wrap cp-aa-path-grid"><div><span>01</span><h2>Identificar la vía aplicable.</h2><p>{p[1]}</p></div><div><span>02</span><h2>Revisar el alcance.</h2><p>{p[2]}</p></div></div></section>
