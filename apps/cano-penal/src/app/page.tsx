...
export default function HomePage() {
  return (
    <PageShell>
      ...
      <section className="cp-section" id="areas"><div className="cp-wrap"><div className="cp-section-head"><div><p className="cp-eyebrow">Especialización penal</p><h2><span className="cp-title-keep">Áreas de</span> <strong>práctica</strong></h2></div></div><div className="cp-areas">{areas.map(([name, href]) => <Link className="cp-area" key={href} href={href}>{name}</Link>)}</div></div></section>
      ...
      <section className="cp-section" id="contacto"><div className="cp-wrap cp-contact"><div><h2>En derecho penal cada minuto cuenta</h2><div className="cp-contact-details"><strong>World Trade Center Ciudad de México</strong><span>Montecito 38, piso 28, oficina 16, colonia Nápoles, Benito Juárez, CDMX</span><a className="cp-text-link cp-map-link" href="https://www.google.com/maps/search/?api=1&amp;query=World%20Trade%20Center%20Ciudad%20de%20M%C3%A9xico%2C%20Montecito%2038%2C%20piso%2028%2C%20oficina%2016%2C%20N%C3%A1poles%2C%20Benito%20Ju%C3%A1rez%2C%20CDMX" target="_blank" rel="noopener noreferrer">Ver ubicación en Google Maps</a><a href={site.phoneHref}>{site.phoneDisplay}</a><a href={`mailto:${site.email}`}>{site.email}</a></div></div><div className="cp-form-panel"><form className="cp-form"><label><span>Nombre</span><input name="nombre" autoComplete="name" /></label><label><span>Teléfono</span><input name="telefono" type="tel" autoComplete="tel" /></label><label className="full"><span>Correo</span><input name="correo" type="email" autoComplete="email" /></label><label className="full"><span>Mensaje</span><textarea name="mensaje" /></label></form><a className="cp-btn cp-btn-solid cp-contact-action" href="https://wa.me/5215560501901" target="_blank" rel="noopener noreferrer">Enviar por WhatsApp</a></div></div></section>
    </PageShell>
  );
}
