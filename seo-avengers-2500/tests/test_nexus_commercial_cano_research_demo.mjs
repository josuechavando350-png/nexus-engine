...
test("CANO's research-backed existing-page demonstration runs on the shared multi-tenant engine", async () => {
  ...
  assert.equal(input.maxImplementationCostMxn, 0, "zero-cost here means an audit of existing paths, not free SEO implementation");
  assert.deepEqual(input.avengersOpportunities.map((row) => row.monthlySearchVolume), [390, 10]);
  ...
  assert.deepEqual(report.evaluatedPortfolios.map((row) => row.worstCaseModeledClientsMilli), [0, 0]);
