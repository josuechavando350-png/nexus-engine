...
async function main() {
  ...
  const [scenario, v3Evidence, v4Evidence] = await Promise.all([
    readJson(options.scenario),
    readJson(options.v3Evidence),
    readJson(options.v4Evidence),
  ]);
  const report = runCommercialDemandTournamentV4(scenario, v3Evidence, v4Evidence);
...
main().catch((error) => {
  process.stderr.write(`NEXUS_COMMERCIAL_DEMAND_TOURNAMENT_V4_ERROR=${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 2;
});
