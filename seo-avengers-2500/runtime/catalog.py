...
IMPLEMENTED_MODULES = tuple(f"M{i}" for i in range(1001, 1601))
RESERVED_MODULES = tuple(f"M{i}" for i in range(1601, 2501))
...
def module_registry() -> Dict[str, Dict[str, Any]]:
    ...
    if sum(1 for v in registry.values() if v["status"] == "IMPLEMENTED_PRODUCTION") != 600:
