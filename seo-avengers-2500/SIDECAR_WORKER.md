...
4. the local deterministic runtime returns the exact M1001-M2500 receipt range (1500 receipts), every receipt remains `SAFE_WHITE_HAT` + `OBSERVE_ONLY`, no runtime receipt is `ERROR`, and M2500 certifies with `SUCCESS` / `NO_FINDING`;
...
If a tenant is killed, disabled or moved to a newer generation while a run is executing, the worker suppresses the receipts. If the evidence snapshot changes during the run, the worker returns `STALE` and suppresses the receipts.
...
- `STALE`: evidence changed while execution was in flight. No receipts are released.
...
The Node sidecar launches only the repository-local Python bridge `sidecar/execute_suite.py` with `shell: false`. The Python bridge imports the existing deterministic `runtime.runner.run_batch_1001_2500()` function. It does not implement a second SEO engine.
