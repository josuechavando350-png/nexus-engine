...
## Deterministic generated JSON normalization

The deterministic-build verifier recursively sorts object keys before hashing
generated `.json` files. JSON object key order and formatting carry no semantic
information, so this is normalization rather than an invariant exception.
