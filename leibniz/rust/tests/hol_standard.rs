...
fn variable(name: &str, ty: Ty) -> E {
    E::Var(name.to_owned(), ty)
}
fn app(f: E, x: E) -> E {
    E::App(Box::new(f), Box::new(x))
}
fn implication(a: E, b: E) -> E {
...
fn arbitrary_predicate_is_applied_inside_a_closed_universal_theorem() {
    ...
    let px = app(
        variable("P", predicate_ty.clone()),
        variable("x", individual()),
    );
    let goal = universal(
        "P",
        predicate_ty.clone(),
        universal("x", individual(), implication(px.clone(), px.clone())),
    );
    let proof = introduction(
        "P",
        predicate_ty,
        introduction(
            "x",
...
fn predicates_of_predicates_are_applied_not_merely_named() {
    ...
    let fp = app(
        variable("F", functional_ty.clone()),
        variable("P", predicate_ty.clone()),
    );
    let goal = universal(
        "F",
...
fn universal_elimination_instantiates_a_new_lambda_not_in_any_catalog() {
    ...
    let universal_goal = universal(
        "P",
        predicate_ty.clone(),
        implication(pa.clone(), pa.clone()),
    );
    let universal_proof = introduction(
        "P",
        predicate_ty,
        D::ImpIntro {
            assumption: pa,
    ...
    prove_closed(
        &goal,
        &D::ForallElim {
            universal: Box::new(universal_proof),
            witness,
        },
    );
}
...
fn predicate_extensionality_requires_a_checked_pointwise_premise() {
    ...
    let pointwise = universal(
        "x",
        individual(),
        E::Equal(
            Box::new(app(left.clone(), x.clone())),
            Box::new(app(right.clone(), x)),
        ),
    );
