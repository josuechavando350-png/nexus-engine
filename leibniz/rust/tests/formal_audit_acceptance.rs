...
use nexus_leibniz::handoff::{
    Direction, GaussOutcomeV1, GaussProblemV1, GaussResponseV1, HandoffAssurance, HandoffLimits,
    Objective, CONTRACT_VERSION,
};
...
fn trusted() -> SemanticArchive {
    ...
    graph
        .assert_fact(atom("reported"), "ledger:real-record")
        .unwrap();
    graph
        .add_rule(Rule {
            id: "reported_implies_eligible".into(),
            body: vec![Pattern {
...
fn problem(archive: &SemanticArchive) -> GaussProblemV1 {
    GaussProblemV1::prepare(
        archive,
        "question-1",
        150,
        Objective {
            metric: "contacts_rate".into(),
...
fn proven(archive: &SemanticArchive) -> GaussResponseV1 {
    ...
    let Decision::Proven { proof } = archive.graph.infer(&query, Limits::default()) else {
        panic!("real asserted source and Horn rule must produce a proof")
    };
    GaussResponseV1 {
        contract_version: CONTRACT_VERSION,
        problem_id: "question-1".into(),
...
fn estimate() -> GaussResponseV1 {
    GaussResponseV1 {
        ...
        outcome: GaussOutcomeV1::Estimated {
            ...
            claim: "unverified future event".into(),
...
) -> Result<nexus_leibniz::formal_audit::ReadOnlyAudit, String> {
...
fn closed_theorem_needs_real_proof_and_independent_model_replay() {
    let (claim, proof) = theorem();
    assert_eq!(
        audit_closed(&claim, Some(&proof), AuditLimits::default()).unwrap(),
...
fn true_in_all_tested_models_without_proof_is_undetermined() {
    let (claim, _) = theorem();
    assert_eq!(
        audit_closed(&claim, None, AuditLimits::default()).unwrap(),
...
fn missing_constant_and_open_variable_are_refused_not_assumed_false() {
    assert!(audit_closed(
        &E::Const("market_will_rise".into(), Ty::Prop),
        None,
        AuditLimits::default()
    )
    .is_err());
    assert!(audit_closed(
        &E::Var("free".into(), Ty::Prop),
        None,
        AuditLimits::default()
    )
    .is_err());
...
fn finite_model_and_proof_work_limits_fail_closed() {
    ...
    assert!(audit_closed(
        ...
        Some(&proof),
        AuditLimits {
            proof: nexus_leibniz::hol::Budget {
                max_nodes: 1,
                max_reductions: 1
            },
            ..AuditLimits::default()
