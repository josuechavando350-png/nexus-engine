...
fn problem(source: &SemanticArchive) -> GaussProblemV1 {
    GaussProblemV1::prepare(
        ...
        150,
        Objective {
            metric: "reported_contacts_rate".into(),
...
fn proven(source: &SemanticArchive) -> GaussResponseV1 {
    let query = atom();
    let Decision::Proven { proof } = source.graph.infer(&query, Limits::default()) else {
        panic!("asserted source must yield a checked derivation")
    };
    GaussResponseV1 {
        contract_version: CONTRACT_VERSION,
        problem_id: "question-original".into(),
...
fn structurally_accepted_estimate_is_never_promoted_to_proven() {
    ...
    let response = GaussResponseV1 {
        ...
        outcome: GaussOutcomeV1::Estimated {
            ...
            claim: "unverified future outcome".into(),
