...
impl Fuel {
    fn tick(&mut self) -> Result<(), String> {
        self.left = self
            .left
            .checked_sub(1)
            .ok_or("finite-model work budget exhausted")?;
        Ok(())
    }
}
...
impl FiniteModel {
    ...
    fn values(&self, ty: &Ty, fuel: &mut Fuel, depth: usize) -> Result<Vec<Value>, String> {
        fuel.tick()?;
        if depth > 64 {
            return Err("finite-model type depth exceeded".into());
        }
        match ty {
            Ty::Individual => Ok((0..self.individuals).map(Value::Individual).collect()),
            Ty::Prop => {
                if self.max_domain < 2 {
                    return Err("Boolean domain exceeds cap".into());
                }
    ...
    fn ty_of(
        ...
        fuel.tick()?;
        if depth > 128 {
            return Err("finite-model expression depth exceeded".into());
        }
        match e {
            ...
            Expr::Imp(left, right) | Expr::And(left, right) => {
                    ...
                    && self.ty_of(right, env, fuel, depth + 1)? == Ty::Prop
                {
                    ...
                    Err("finite-model ill-typed connective".into())
                }
            }
            Expr::Equal(left, right) => {
                    ...
                    == self.ty_of(right, env, fuel, depth + 1)?
                {
                    ...
                    Err("finite-model ill-typed equality".into())
                }
            }
        }
    }
    ...
    fn eval(
        &self,
        e: &Expr,
        env: &mut Vec<(String, Ty, Value)>,
        fuel: &mut Fuel,
        depth: usize,
    ) -> Result<Value, String> {
        fuel.tick()?;
        if depth > 128 {
            return Err("finite-model evaluation depth exceeded".into());
        }
        let child = depth + 1;
        match e {
            Expr::Var(name, _) => env
                ...
                .ok_or("finite-model unbound variable".into()),
            Expr::Const(name, _) => self
                .constants
                .get(name)
            ...
            Expr::App(function, argument) => {
                let mut types: Vec<(String, Ty)> =
                    env.iter().map(|(n, t, _)| (n.clone(), t.clone())).collect();
