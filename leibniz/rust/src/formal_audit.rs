...
pub fn audit_closed(
    ...
    Ok(if certificate.is_some() {
        FormalAudit::ClosedTheoremChecked {
            models_checked: limits.max_individuals,
        }
    } else {
        FormalAudit::Undetermined {
            models_checked: limits.max_individuals,
        }
    })
}
...
pub fn audit_trusted_handoff(
    ...
    let handoff =
        response.assess_against_trusted_source(problem, trusted_archive, handoff_limits)?;
