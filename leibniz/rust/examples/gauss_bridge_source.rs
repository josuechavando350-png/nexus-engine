...
fn run() -> Result<(), String> {
    ...
    if rates.iter().any(|rate| !rate.is_finite() || *rate < 0.0 || *rate > 1e9) {
        return Err("fixture rates must be bounded and nonnegative".into());
    }
    let mut graph = Graph::new();
    for id in ["source", "left", "right"] {
        graph.add_entity(Entity {
            id: id.into(), category: "SYNTHETIC_TEST_ONLY".into(),
    ...
    let annotations = ["synthetic:left", "synthetic:right"].into_iter()
