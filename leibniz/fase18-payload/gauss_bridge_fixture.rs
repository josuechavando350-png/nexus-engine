...
fn fixture() -> Result<SemanticArchive, String> {
    ...
    for id in ["fixture-client", "fixture-market"] {
        graph.add_entity(Entity { id: id.into(), category: "Synthetic".into(), attributes: HashMap::new() })?;
    }
    graph.add_flow(Flow {
        from_entity: "fixture-client".into(), to_entity: "fixture-market".into(), rate_of_transfer: 2.5,
    })?;
...
fn run() -> Result<(), String> {
    ...
    let outcome = (|| -> Result<(), String> {
        ...
        if restored.to_bytes()? != canonical { return Err("fixture changed on disk".into()); }
        let objective = Objective { metric: "synthetic_rate".into(),
            subject_entity_id: "fixture-client".into(),
            unit: restored.snapshot.flows[0].annotation.unit.clone(),
            direction: Direction::Maximize };
        let request = GaussProblemV1::prepare(&restored, "leibniz-gauss-fixture", 150,
            objective.clone(), HandoffLimits::default())?;
        let rate = request.measured_flow_sum_against_trusted_source(
            "fixture-client", "fixture-market", &objective.unit,
            &restored, HandoffLimits::default())?;
        if rate.evidence_ids.len() != 1 || rate.evidence_ids[0] != "fixture:rate"
        ...
        println!("LEIBNIZ_SYNTHETIC_RATE_V1\t{}\t{}\t{}\t{}\t{}",
            request.problem_id, request.as_of_utc_ms, rate.value,
            rate.unit.symbol, rate.evidence_ids[0]);
        Ok(())
    })();
