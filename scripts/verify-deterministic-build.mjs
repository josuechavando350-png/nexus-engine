...
const canonicalPath = (path, buildId) => buildId ? path.replaceAll(buildId, "<NEXUS_EPHEMERAL_NEXT_BUILD_ID>") : path;
const canonicalNextBytes = (path, bytes, buildId) => {
  if (!path.includes("/.next/")) return bytes;
  const basename = path.split("/").at(-1);
  if (basename === "BUILD_ID") return Buffer.from("<NEXUS_EPHEMERAL_NEXT_BUILD_ID>\n", "utf8");
  let canonical = buildId ? Buffer.from(bytes.toString("utf8").replaceAll(buildId, "<NEXUS_EPHEMERAL_NEXT_BUILD_ID>"), "utf8") : bytes;
  if (!["prerender-manifest.json","server-reference-manifest.json","pages-manifest.json"].includes(basename)) return canonical;
  let parsed; try { parsed = JSON.parse(canonical.toString("utf8")); } catch { return canonical; }
...
const fileEntries = (paths) => paths.map((rawPath) => { const bytes=readFileSync(join(root,rawPath)); const buildId=buildIdForPath(rawPath); const path=canonicalPath(rawPath,buildId); const canonicalBytes=canonicalNextBytes(rawPath,bytes,buildId); return {path,size:canonicalBytes.length,sha256:createHash("sha256").update(canonicalBytes).digest("hex"),rawPath,rawSize:bytes.length,rawSha256:createHash("sha256").update(bytes).digest("hex")}; });
