...
function assertFormalLinkage(input: CreateFormalExperienceProofInput): void {
  ...
  if (input.visual.subject !== subject || input.topology.certificate.subject !== subject || input.semantics.certificate.subject !== subject) {
...
export function formalExperienceProofDigest(input: CreateFormalExperienceProofInput): string {
  ...
  return digestValue({
    authority: "NEXUS_FORMAL_EXPERIENCE_PROOF_V1",
    version: 1,
...
function assertLinkage(input: CreateExperienceProofInput): string {
  ...
  if (input.evidenceAnchor.formalDigest !== formalDigest) throw new Error("Signed evidence is not bound to the carried formal Motor 1-3 proof");
  return formalDigest;
}
...
function makeClaims(input: CreateExperienceProofInput): readonly ExperienceProofClaim[] {
  ...
  const evidence = claim({ kind: "SIGNED_EVIDENCE", subject: input.artifact.subject, status: "VERIFIED", evidenceDigest: input.evidenceAnchor.anchorDigest, dependencies: [artifact.claimId, visual.claimId, topology.claimId, semantics.claimId] });
