export const FRICTION_FEATURE_CONTRACT_ID = "NEXUS_WEB_FRICTION_SIGNALS_V1" as const;

export type FrictionPointerClass = "COARSE" | "FINE";
...
export interface FrictionSnapshot {
  ...
  readonly pointerClass: FrictionPointerClass;
...
const MODEL_KEYS = ["coefficients", "featureContractId", "intercept", "lowRiskMax", "mediumRiskMax", "modelId", "schemaVersion", "sourceDigest"].sort().join(",");
const COEFFICIENT_KEYS = [
  "coarsePointerIndicator",
  "interactionLatency",
  "longTaskRate",
  "repeatedActionRatio",
  "scrollDeficit",
...
export function parseFrictionSnapshot(value: unknown): FrictionSnapshot {
  ...
  if (input.featureContractId !== FRICTION_FEATURE_CONTRACT_ID) throw new Error("friction snapshot feature contract is invalid");
  if (!(input.pointerClass === "COARSE" || input.pointerClass === "FINE")) throw new Error("friction snapshot pointerClass is invalid");
  return Object.freeze({
    schemaVersion: 1,
    featureContractId: FRICTION_FEATURE_CONTRACT_ID,
    pointerClass: input.pointerClass,
...
export function parseFrictionProbabilityModel(value: unknown): FrictionProbabilityModel {
  ...
  if (input.featureContractId !== FRICTION_FEATURE_CONTRACT_ID) throw new Error("friction probability feature contract is invalid");
  ...
  const coefficients = Object.freeze({
    ...
    coarsePointerIndicator: boundedNumber(coefficientInput.coarsePointerIndicator, "coarsePointerIndicator coefficient", -32, 32),
  });
  return Object.freeze({
    schemaVersion: 1,
    featureContractId: FRICTION_FEATURE_CONTRACT_ID,
...
export function scoreFrictionAbandonment(snapshotInput: unknown, modelInput: unknown): FrictionScore {
  ...
  const linear = model.intercept
    ...
    + model.coefficients.coarsePointerIndicator * (snapshot.pointerClass === "COARSE" ? 1 : 0);
  ...
  return Object.freeze({
    ...
    pointerClass: snapshot.pointerClass,
