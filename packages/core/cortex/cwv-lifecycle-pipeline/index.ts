...
export async function executeCwvBuildOptimizationPlan(plan: CwvBuildOptimizationPlan, adapter: CwvBuildOptimizationAdapter): Promise<readonly CwvOptimizationReceipt[]> {
  ...
  for (const action of plan.actions) {
    ...
    if (action.required && receipt.status !== "APPLIED" && receipt.beforeDigest === receipt.afterDigest) throw new Error(`required CWV action ${action.actionId} was not applied`);
...
export function certifyCwvLifecycleOptimization(input: { plan: CwvBuildOptimizationPlan; receipts: readonly CwvOptimizationReceipt[]; before: CwvMetricsSnapshot; after: CwvMetricsSnapshot; proofs: CwvProofDigest; guardrails: CwvRegressionGuardrails }): CwvLifecycleCertification {
  ...
  const ids = new Set(input.receipts.map((receipt) => receipt.actionId));
  for (const action of input.plan.actions) if (!ids.has(action.actionId)) throw new Error(`CWV receipt missing for ${action.actionId}`);
...
export function evaluateCwvRuntimeOptimization(sample: CwvLifecycleSample, thresholds?: CwvLifecycleThresholds) {
...
export function parseCwvEdgePolicy(value: unknown): CwvEdgePolicy {
  ...
  const routes = raw.routes.map((entry, index) => {
    ...
    const route = entry as Record<string, unknown>; if (Object.keys(route).sort().join(",") !== "cacheControl,lcpPreloadAs,lcpPreloadPath,path" || typeof route.path !== "string" || !PATH.test(route.path) || seen.has(route.path) || typeof route.cacheControl !== "string" || route.cacheControl.length > 256) throw new TypeError(`CWV edge route ${index} contract is invalid`); seen.add(route.path);
