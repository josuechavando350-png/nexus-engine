...
import {
  ...
  type SceneInput,
  type Viewport,
} from "@nexus/capture-visual-regression-v2";
import { captureSceneAtNavigationUrl } from "@nexus/capture-visual-regression-v2/runtime-navigation";
...
function parseEnvelope(value: unknown, projectId: string): BaselineEnvelope {
  ...
  if (record.projectId !== projectId) throw new Error(`visual baseline projectId ${String(record.projectId ?? "missing")} does not match ${projectId}`);
  ...
  const entries = record.baselines.map((raw, index) => {
    ...
    const scene = createScene(item.scene as SceneInput);
    ...
    const viewport = createViewport(String(viewportRaw.name ?? ""), Number(viewportRaw.width), Number(viewportRaw.height));
    ...
    return Object.freeze({ id, route: runtimeRoute, scene: { ...item.scene } as SceneInput, browserName, viewport, screenshotPath, baseline });
  });
  ...
  if (new Set(identityKeys).size !== identityKeys.length) throw new Error("visual baseline scene/browser/viewport identities must be unique");
...
function buildUnavailable(execution: BuildExecution): boolean {
  return Boolean(execution.unavailableReason || execution.exitCode === null);
}
...
export async function runVisualComparator(input: VisualComparatorInput, dependencies: VisualComparatorDependencies): Promise<VisualComparatorExecution> {
  ...
  const envelope = parseEnvelope(JSON.parse(envelopeFile.bytes.toString("utf8")), dependencies.project.slug);
  const baselineFiles = new Map<string, { absolute: string; relativePath: string; sha256: string }>();
  for (const entry of envelope.baselines) {
    ...
    baselineFiles.set(entry.id, { absolute: join(dependencies.root, verified.relativePath), relativePath: verified.relativePath, sha256: verified.sha256 });
  }
  ...
  if (buildUnavailable(execution)) throw new Error(`VISUAL_BUILD_UNAVAILABLE: ${execution.unavailableReason ?? "build did not execute"}`);
  if (execution.exitCode !== 0 || !execution.manifest) throw new Error(`VISUAL_BUILD_FAILED: build exited ${execution.exitCode ?? "without status"}`);
  ...
  if (!/^[a-f0-9]{64}$/u.test(buildDigest)) throw new Error("visual build outputDigest is invalid");
  if (!dependencies.ssimulacra2Path || !dependencies.ssimulacra2Path.trim()) throw new Error("PERCEPTUAL_COMPARATOR_UNAVAILABLE: NEXUS_SSIMULACRA2_PATH is not configured");

  const perceptual = createSsimulacra2Comparator(dependencies.ssimulacra2Path);
  const comparisons = await withProjectServer(dependencies.root, dependencies.project, async (origin) => {
    ...
    for (const entry of envelope.baselines) {
      const scene = createScene(entry.scene);
      const baselineFile = baselineFiles.get(entry.id)!;
      const current = await captureSceneAtNavigationUrl({
        scene,
        navigationUrl: runtimeUrl(origin, entry.route),
        browserName: entry.browserName,
        viewport: entry.viewport,
        revision: input.sourceSha,
        buildDigest,
        outDir: join(dependencies.artifactRoot, dependencies.requestId, "visual-regression", entry.id),
      });
      const report = await compareCapture({
        baseline: entry.baseline,
        baselinePath: baselineFile.absolute,
        current,
        policy: scene.policy,
        perceptual,
        outDir: join(dependencies.artifactRoot, dependencies.requestId, "visual-regression", entry.id, "diff"),
      });
      results.push(Object.freeze({
        ...
        browserName: entry.browserName,
        viewport: entry.viewport,
        approvalReference: entry.baseline.approvalReference,
        baselineDigest: entry.baseline.digest,
        baselineScreenshotSha256: entry.baseline.screenshotSha256,
        currentScreenshotSha256: current.record.screenshotSha256,
        captureDigest: current.record.digest,
        report,
        currentScreenshotPath: current.path,
        diffPath: report.diffPath ?? null,
      }));
    }
  ...
  return Object.freeze({
    projectId: dependencies.project.slug,
    sourceSha: input.sourceSha,
    buildDigest,
    buildManifestSha256: execution.manifest.manifestSha256,
    baselineEnvelopePath: envelopeFile.relativePath,
    baselineEnvelopeSha256: envelopeFile.sha256,
    comparisons,
    build: execution,
  });
}
