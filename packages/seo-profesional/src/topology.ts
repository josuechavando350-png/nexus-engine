...
export const SEO_PROFESSIONAL_STRATEGIES: readonly SeoProfessionalStrategyDefinition[] = Object.freeze([
  Object.freeze({
    ...
    implementationRef: "@nexus/ontology:SeoProfesionalDetectorDeTrampas",
    responsibility: "invalid-traffic assessment and qualified offline conversion feedback",
  }),
  Object.freeze({
    number: 2,
    id: "cazador-con-lupa",
    implementationRef: "@nexus/ontology:SeoProfesionalCazadorConLupa",
    responsibility: "search-term evidence and exact-match materialization",
  }),
  Object.freeze({
    number: 3,
    id: "camaleon-web",
    implementationRef: "@nexus/core/cortex/ad-context-edge-personalization",
...
export const SEO_PROFESSIONAL_CONNECTIONS: readonly SeoProfessionalConnection[] = Object.freeze([
  Object.freeze({
    from: 1,
    to: 2,
    channel: "QUALIFIED_CONVERSION_FEEDBACK",
    boundary: "GOOGLE_ADS",
  }),
  Object.freeze({
    from: 2,
    to: 3,
    channel: "PAID_SEARCH_TRAFFIC",
    boundary: "GOOGLE_ADS",
  }),
  Object.freeze({
    from: 3,
    to: 1,
    channel: "ATTRIBUTED_LANDING_FEEDBACK",
    boundary: "WEB_REQUEST",
  }),
] as const);
...
export function assertConnectedSeoProfessionalTopology(
  ...
  const forward = reachable(start, connections, false);
  const backward = reachable(start, connections, true);
  if (forward.size !== strategies.length || backward.size !== strategies.length) {
