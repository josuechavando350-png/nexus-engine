...
import { certifyDelivery } from "./delivery-certification";
...
describe("delivery certification", () => {
  ...
  it("rejects cross-project certification even with a valid signature", () => {
    const { signed, publicKey } = signedEvidence();
    const result = certifyDelivery({ signedEvidence: signed, publicKey, sourceRevision: revision, tenantId: "tenant-a", projectId: "project-b" });
    expect(result.certified).toBe(false);
  ...
  it("rejects a bundle signed by an untrusted key", () => {
    ...
    expect(() => certifyDelivery({ signedEvidence: signed, publicKey: attacker.publicKey, sourceRevision: revision, tenantId: "tenant-a", projectId: "project-a" })).toThrow(/signature verification failed/);
  });
});
