...
describe("AI proposal to Action execution boundary", () => {
  it("never lets an AI proposal bypass policy-owned risk or human approval", async () => {
    ...
    const provider: AIProviderPort = { providerId: "fake-ai", propose: async () => ({ actionId: "action.customer.create", targetId: "customer-ai", rationale: "create the requested customer", risk: "LOW" }) };
    const proposal = await new ControlledAIBoundary(provider).propose({ requestId: "ai-e2e-1", principal, scope: active.scope, schema: active, intent: "Create Ada", allowedActionIds: ["action.customer.create"], maxInputChars: 100 });
    expect(proposal.executable).toBe(false); expect(proposal.risk).toBe("LOW");
    ...
    expect(result.status).toBe("DENIED"); expect(result.reason).toContain("approval"); expect(transactions.getObject(active.scope, "customer-ai")).toBeUndefined(); expect(audit.list(active.scope)[0]?.risk).toBe("HIGH");
  });
});
