...
function parseRemoteEvent(value: unknown): DurableFormEventRecord {
  ...
  if (typeof raw.stream !== "string" || typeof raw.eventId !== "string" || !ID.test(raw.eventId) || typeof raw.occurredAt !== "string" || typeof raw.sequence !== "number" || !Number.isSafeInteger(raw.sequence) || raw.sequence < 1) throw new Cortex20WorkerError("INTEGRITY_FAILURE", "remote event identity is invalid");
...
export class HttpCortex17FormEventClient {
  ...
  constructor(origin: URL, private readonly readToken: string, private readonly writeToken: string, private readonly timeoutMs = 10_000) {
    ...
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 60_000) throw new Cortex20WorkerError("INVALID_INPUT", "event service timeout is invalid");
