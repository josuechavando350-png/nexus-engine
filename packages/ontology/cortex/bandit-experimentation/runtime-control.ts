...
const EVENT = Object.freeze({
  ...
  authorizationSource: "cortex.bandit.control_event.authorization_source",
  commandId: "cortex.bandit.control_event.command_id",
  controlPolicyDigest: "cortex.bandit.control_event.control_policy_digest",
  evidenceDigest: "cortex.bandit.control_event.evidence_digest",
...
export interface CortexBanditRuntimeControlAuthorization {
  readonly source: "EXTERNAL_CONTROL_PLANE";
...
export interface CortexBanditRuntimeControlEvent {
  ...
  readonly authorization?: CortexBanditRuntimeControlAuthorization;
...
function identifier(value: string, field: string, code: "INVALID_INPUT" | "INTEGRITY_FAILURE" = "INVALID_INPUT"): string { if (typeof value !== "string") throw new CortexBanditRuntimeControlError(code, `${field} must be a string`); const normalized = value.trim(); if (!IDENTIFIER.test(normalized)) throw new CortexBanditRuntimeControlError(code, `${field} is malformed`); return normalized; }
function policyDigest(value: string, field = "policyDigest", code: "INVALID_INPUT" | "INTEGRITY_FAILURE" = "INVALID_INPUT"): string { if (typeof value !== "string" || !DIGEST.test(value)) throw new CortexBanditRuntimeControlError(code, `${field} is malformed`); return value; }
...
function optionalProperty(id: string, name: string, valueKind: "STRING" | "NUMBER" | "DATETIME", immutable = false) { return { id, name, valueKind, cardinality: "OPTIONAL", unique: false, immutable } as const; }
...
function controlSchema(scope: OntologyScope): ValidatedSchema {
  const schema: SchemaVersion = {
    ...
    properties: [
      ...
      property(EVENT.experimentId, "BanditControlEventExperimentId", "STRING", true), property(EVENT.policyDigest, "BanditControlEventPolicyDigest", "STRING", true), property(EVENT.fromMode, "BanditControlEventFromMode", "STRING", true), property(EVENT.toMode, "BanditControlEventToMode", "STRING", true), property(EVENT.reason, "BanditControlEventReason", "STRING", true), property(EVENT.changedAt, "BanditControlEventChangedAt", "DATETIME", true), property(EVENT.targetRevision, "BanditControlEventTargetRevision", "NUMBER", true), optionalProperty(EVENT.authorizationSource, "BanditControlEventAuthorizationSource", "STRING", true), optionalProperty(EVENT.commandId, "BanditControlEventCommandId", "STRING", true), optionalProperty(EVENT.controlPolicyDigest, "BanditControlEventControlPolicyDigest", "STRING", true), optionalProperty(EVENT.evidenceDigest, "BanditControlEventEvidenceDigest", "STRING", true), property(EVENT.digest, "BanditControlEventDigest", "STRING", true),
...
function optionalStringProperty(record: ObjectRecord, key: string): string | undefined { const value = record.properties[key]; if (value === undefined || value === null) return undefined; if (typeof value !== "string") throw new CortexBanditRuntimeControlError("INTEGRITY_FAILURE", `record ${record.id} has invalid ${key}`); return value; }
...
function eventProperties(input: Omit<CortexBanditRuntimeControlEvent, "digest">): Readonly<Record<string, JsonValue>> {
  const base: Record<string, JsonValue> = { [EVENT.experimentId]: input.experimentId, [EVENT.policyDigest]: input.policyDigest, [EVENT.fromMode]: input.fromMode, [EVENT.toMode]: input.toMode, [EVENT.reason]: input.reason, [EVENT.changedAt]: input.changedAt, [EVENT.targetRevision]: input.targetRevision };
  if (input.authorization !== undefined) {
    base[EVENT.authorizationSource] = input.authorization.source;
    base[EVENT.commandId] = input.authorization.commandId;
    base[EVENT.controlPolicyDigest] = input.authorization.controlPolicyDigest;
    if (input.authorization.evidenceDigest !== null) base[EVENT.evidenceDigest] = input.authorization.evidenceDigest;
  }
  base[EVENT.digest] = eventDigest(input);
  return Object.freeze(base);
}

function authorization(value: CortexBanditRuntimeControlAuthorization, code: "INVALID_INPUT" | "INTEGRITY_FAILURE" = "INVALID_INPUT"): CortexBanditRuntimeControlAuthorization {
  if (!value || typeof value !== "object" || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype) throw new CortexBanditRuntimeControlError(code, "runtime control authorization must be a plain object");
  if (value.source !== "EXTERNAL_CONTROL_PLANE") throw new CortexBanditRuntimeControlError(code, "runtime control authorization source is invalid");
  const commandId = identifier(value.commandId, "authorization.commandId", code);
  const controlPolicyDigest = policyDigest(value.controlPolicyDigest, "authorization.controlPolicyDigest", code);
  const evidenceDigest = value.evidenceDigest === null ? null : policyDigest(value.evidenceDigest, "authorization.evidenceDigest", code);
  return Object.freeze({ source: "EXTERNAL_CONTROL_PLANE", commandId, controlPolicyDigest, evidenceDigest });
}
...
function parseEvent(record: ObjectRecord): CortexBanditRuntimeControlEvent {
  ...
  const core = { experimentId: identifier(stringProperty(record, EVENT.experimentId), "stored experimentId", "INTEGRITY_FAILURE"), policyDigest: policyDigest(stringProperty(record, EVENT.policyDigest), "stored policyDigest", "INTEGRITY_FAILURE"), fromMode: mode(stringProperty(record, EVENT.fromMode), "INTEGRITY_FAILURE"), toMode: mode(stringProperty(record, EVENT.toMode), "INTEGRITY_FAILURE"), reason: reason(stringProperty(record, EVENT.reason), "INTEGRITY_FAILURE"), changedAt: canonicalUtc(stringProperty(record, EVENT.changedAt), "stored changedAt", "INTEGRITY_FAILURE"), targetRevision: positiveRevision(numberProperty(record, EVENT.targetRevision), "stored targetRevision", "INTEGRITY_FAILURE") };
  const source = optionalStringProperty(record, EVENT.authorizationSource);
  const commandId = optionalStringProperty(record, EVENT.commandId);
  const controlPolicyDigest = optionalStringProperty(record, EVENT.controlPolicyDigest);
  const evidenceDigest = optionalStringProperty(record, EVENT.evidenceDigest);
  let storedAuthorization: CortexBanditRuntimeControlAuthorization | undefined;
  if (source === undefined) {
    if (commandId !== undefined || controlPolicyDigest !== undefined || evidenceDigest !== undefined) throw new CortexBanditRuntimeControlError("INTEGRITY_FAILURE", "control event has incomplete authorization provenance");
  } else {
    if (commandId === undefined || controlPolicyDigest === undefined) throw new CortexBanditRuntimeControlError("INTEGRITY_FAILURE", "control event authorization provenance is incomplete");
    storedAuthorization = authorization({ source: source as "EXTERNAL_CONTROL_PLANE", commandId, controlPolicyDigest, evidenceDigest: evidenceDigest ?? null }, "INTEGRITY_FAILURE");
  }
  const digestInput = storedAuthorization === undefined ? core : { ...core, authorization: storedAuthorization };
  const observed = stringProperty(record, EVENT.digest); if (observed !== eventDigest(digestInput)) throw new CortexBanditRuntimeControlError("INTEGRITY_FAILURE", "control event digest mismatch"); return Object.freeze({ ...core, ...(storedAuthorization === undefined ? {} : { authorization: storedAuthorization }), digest: observed });
}
...
export class CortexBanditRuntimeController {
  ...
  set(input: SetCortexBanditRuntimeControlInput): CortexBanditRuntimeControlState {
    ...
    const nextAuthorization = input.authorization === undefined ? undefined : authorization(input.authorization);
    ...
    const eventCore = { experimentId: this.experimentId, policyDigest: this.policyDigest, fromMode: current.mode, toMode: nextMode, reason: nextReason, changedAt, targetRevision: nextRevision, ...(nextAuthorization === undefined ? {} : { authorization: nextAuthorization }) } as const;
