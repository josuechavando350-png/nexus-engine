...
function armsFrom(value: unknown): readonly CortexBanditArmDefinition[] {
  ...
  return Object.freeze(value.map((item, index) => {
    ...
    return Object.freeze({
      armId: requiredString(object, "armId", `arms[${index}]`),
      payload: Object.freeze({ ...payload }) as CortexBanditArmDefinition["payload"],
      minTrafficShare: object.minTrafficShare as number,
      maxTrafficShare: object.maxTrafficShare as number,
    });
  }));
}
...
export function parseCortexBanditProductionConfig(value: unknown): CortexBanditProductionConfig {
  ...
  const experiments = experimentsInput.map((item, index): CortexBanditProductionExperimentConfig => {
    ...
    new ServerSideContextualBanditEngine({
      getObject: () => undefined,
      getRelationship: () => undefined,
      transact: () => ({ committed: true, objectIds: [], relationshipIds: [] }),
    }, scopeFrom(object.scope), experimentId, validatedPolicy, arms);
  ...
  return Object.freeze({ version: 1, scope: scopeFrom(object.scope), experiments: Object.freeze(experiments) });
}
...
async function readJsonBody(request: IncomingMessage): Promise<unknown> {
  if (!jsonContentType(request)) throw Object.assign(new Error("content-type must be application/json"), { statusCode: 415 });
  let bytes = 0;
  const chunks: Buffer[] = [];
  for await (const chunk of request) {
    ...
    if (bytes > MAX_BODY_BYTES) {
      request.destroy();
      throw Object.assign(new Error(`request body exceeds ${MAX_BODY_BYTES} bytes`), { statusCode: 413 });
    }
    chunks.push(buffer);
  }
  if (bytes === 0) throw Object.assign(new Error("request body is required"), { statusCode: 400 });
  try {
    return JSON.parse(Buffer.concat(chunks).toString("utf8")) as unknown;
  } catch {
    throw Object.assign(new Error("request body contains malformed JSON"), { statusCode: 400 });
  }
}
...
function exactRequestBody(value: unknown, allowed: readonly string[], label: string): Record<string, unknown> {
...
export function createCortexBanditHttpRuntime(options: CortexBanditHttpRuntimeOptions): CortexBanditHttpRuntime {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (action === "select") {
        operation = "SELECT";
        const input = exactRequestBody(body, ["requestId", "context", "eligibleArmIds"], "selection request");
        if (typeof input.requestId !== "string") throw Object.assign(new Error("requestId must be a string"), { statusCode: 400 });
        if (!Array.isArray(input.eligibleArmIds) || !input.eligibleArmIds.every((item) => typeof item === "string")) throw Object.assign(new Error("eligibleArmIds must be a string array"), { statusCode: 400 });
        const context = plainObject(input.context, "selection request.context") as CortexBanditContext;
        ...
        operation = "OUTCOME";
        const input = exactRequestBody(body, ["decisionId", "converted", "economicValue", "outcomeAt"], "outcome request");
        if (typeof input.decisionId !== "string" || typeof input.converted !== "boolean" || typeof input.economicValue !== "number" || typeof input.outcomeAt !== "string") {
          throw Object.assign(new Error("outcome request fields are malformed"), { statusCode: 400 });
        }
        ...
        const input = exactRequestBody(body, ["expectedRevision", "mode", "reason", "changedAt"], "control request");
        if (typeof input.expectedRevision !== "number" || typeof input.mode !== "string" || typeof input.reason !== "string") {
          throw Object.assign(new Error("control request fields are malformed"), { statusCode: 400 });
        }
        if (input.changedAt !== undefined && typeof input.changedAt !== "string") throw Object.assign(new Error("changedAt must be a string"), { statusCode: 400 });
