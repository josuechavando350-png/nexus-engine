...
The runtime persists a single integrity-digested control object per ontology scope. The control stores only non-secret policy configuration, the active policy, the immediately previous policy, a monotonic generation and a non-reversible HMAC key-continuity verifier. The pseudonymization secret itself is never written to control state.

On every runtime start, the supplied pseudonymization secret is checked against that durable verifier for the active `pseudonymizationKeyId`. Reusing the same key ID with a different secret fails closed before ingestion, preventing silent session fragmentation and replay/idempotency drift. CORTEX #6 also rejects changing `pseudonymizationKeyId` through policy activation; explicit key rotation and lifecycle remain the responsibility of CORTEX #26.
...
Control state is durable through the same `OntologyTransactionPort`; SQLite restart tests prove that an activated policy survives close/reopen and can still be rolled back afterward. Dedicated restart coverage also proves that a mismatched pseudonymization secret is rejected without mutating durable state.
...
Direct construction of the lower-level trackers remains available for tests and specialized embedding, but deployments claiming GREEN-PROD for #6 should use `CortexBehavioralSignalRuntime` so kill, rollback, privacy-key continuity and unified observability cannot be accidentally bypassed.
