...
type Cortex19OutboxMode = "ACTIVE" | "OBSERVE_ONLY" | "KILLED";

export class Cortex19OutboxError extends Error {
  constructor(public readonly code: "INVALID_INPUT" | "DELIVERY_FAILURE" | "INTEGRITY_FAILURE" | "KILLED", message: string) {
    super(message);
...
function safeMode(provider: () => Cortex19OutboxMode): Cortex19OutboxMode {
  try {
    const mode = provider();
...
function assertActive(provider: () => Cortex19OutboxMode, boundary: string): void {
  if (safeMode(provider) !== "ACTIVE") throw new Cortex19OutboxError("KILLED", `CORTEX #19 outbox disabled ${boundary}`);
}
...
export class SqliteCouponOutboxDispatcher {
  ...
  constructor(
    ...
    private readonly readMode: () => Cortex19OutboxMode,
    private readonly timeoutMs = 5_000,
  ) {
    if (!databasePath || typeof readMode !== "function") throw new Cortex19OutboxError("INVALID_INPUT", "databasePath and readMode are required");
  ...
  async flush(limit = 100): Promise<{ attempted: number; delivered: number; remaining: number }> {
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 1_000) throw new Cortex19OutboxError("INVALID_INPUT", "outbox flush request is invalid");
    assertActive(this.readMode, "before batch selection");
    ...
    for (const row of rows) {
      ...
      // Final guard immediately before the external side effect. There is no await between this
      // durable control read and fetch(), so a killed runtime cannot knowingly start a new POST.
      assertActive(this.readMode, "at final delivery boundary");
      ...
      // If control changed while the remote call was in flight, leave the event pending. The CORTEX
      // #17 endpoint is eventId-idempotent, so a later ACTIVE retry is safe and no false ack is stored.
      assertActive(this.readMode, "before durable acknowledgement");
