...
export class SqliteCouponIssuer {
  ...
  flushEvents(stream: DurableCouponEventWriter, limit = 100): number {
    if (!stream || stream.idempotentByEventId !== true || typeof stream.append !== "function" || !Number.isSafeInteger(limit) || limit < 1 || limit > 1_000) throw new Cortex19Error("INVALID_INPUT", "flush request requires an eventId-idempotent writer and valid limit");
    ...
    for (const row of rows) {
      ...
      const marked = this.db.prepare("UPDATE cortex19_outbox SET sent=1 WHERE event_id=? AND sent=0").run(String(row.event_id));
      if (marked.changes !== 1) throw new Cortex19Error("INTEGRITY_FAILURE", "outbox acknowledgement raced or disappeared");
      sent += 1;
    }
    return sent;
  }
