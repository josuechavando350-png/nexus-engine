...
describe("CORTEX #19 durable issuance", () => {
  ...
  it("flushes only to writers that explicitly guarantee idempotency by eventId", () => {
    ...
    const writer = { idempotentByEventId: true as const, append(event: DurableCouponEventInput) { published.push(event); return { sequence: published.length }; } };
    expect(issuer.flushEvents(writer)).toBe(1); expect(issuer.flushEvents(writer)).toBe(0);
    expect(published).toHaveLength(1); expect(JSON.stringify(published[0])).not.toContain(request.subjectHash); expect(JSON.stringify(published[0])).toContain(request.probabilityEvidence.modelDigest);
    expect(() => issuer.flushEvents({ append() { return undefined; } } as never)).toThrowError(/idempotent writer/u);
    issuer.close();
  });
});
