...
export function startCortex19ProductionRuntime(env: NodeJS.ProcessEnv = process.env): Cortex19ProductionRuntime {
  ...
  const outbox = new SqliteCouponOutboxDispatcher(databasePath, eventEndpoint(env), eventToken, () => control.read().mode, integerEnv(env, "NEXUS_CORTEX_19_EVENT_TIMEOUT_MS", 5_000, 100, 30_000));
