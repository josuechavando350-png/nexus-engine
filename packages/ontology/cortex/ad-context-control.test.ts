...
describe("CORTEX ad-context durable control plane",()=>{
  it("persists kill across restart, separates privileges and aggregates safe decisions",async()=>{
    const dir=mkdtempSync(join(tmpdir(),"nexus-ad-context-"));dirs.push(dir);const db=join(dir,"state.sqlite");const now=Date.parse("2026-09-05T23:45:00.000Z");
  ...
  it("refuses policy-config weakening, cross-policy observations and unknown telemetry fields",async()=>{
    ...
    expect(()=>controller.observe({policyId:"other-policy",mode:"OBSERVE_ONLY",channel:"PAID_SEARCH",reason:"OBSERVE_ONLY_MATCH",applied:false,observedAt:new Date().toISOString()})).toThrow(/does not match controller policy/i);
    const production=createAdContextControlServer({controller,edgeToken,controlToken});const base=await listen(production.server);
    const mismatch=await fetch(`${base}/v1/ad-context/observe`,{method:"POST",headers:{authorization:`Bearer ${edgeToken}`,"content-type":"application/json"},body:JSON.stringify({policyId:"other-policy",mode:"OBSERVE_ONLY",channel:"PAID_SEARCH",reason:"OBSERVE_ONLY_MATCH",applied:false,observedAt:new Date().toISOString()})});expect(mismatch.status).toBe(400);expect(await mismatch.json()).toEqual({error:"POLICY_MISMATCH"});
    const response=await fetch(`${base}/v1/ad-context/observe`,{method:"POST",headers:{authorization:`Bearer ${edgeToken}`,"content-type":"application/json"},body:JSON.stringify({policyId:"cano-paid-landing-v1",mode:"OBSERVE_ONLY",channel:"PAID_SEARCH",reason:"OBSERVE_ONLY_MATCH",applied:false,observedAt:new Date().toISOString(),gclid:"forbidden"})});expect(response.status).toBe(400);await production.close();store.close();
  });

  it("fails closed if durable control/history belong to a different policy identity",()=>{
    const dir=mkdtempSync(join(tmpdir(),"nexus-ad-context-"));dirs.push(dir);const db=join(dir,"state.sqlite");const now=Date.parse("2026-09-05T23:45:00.000Z");
    const store=new SqliteOntologyTransactionStore(db);const original=new AdContextRuntimeController(store,scope,"cano-edge","cano-paid-landing-v1","ACTIVE",()=>now);original.set({expectedRevision:0,mode:"KILLED",reason:"certified containment"});
    const rotated=new AdContextRuntimeController(store,scope,"cano-edge","cano-paid-landing-v2","ACTIVE",()=>now+1000);
    expect(()=>rotated.current()).toThrow(/identity\/revision mismatch/i);
    expect(()=>rotated.history()).toThrow(/identity\/revision mismatch/i);
    expect(()=>rotated.observe({policyId:"cano-paid-landing-v2",mode:"KILLED",channel:"DIRECT_OR_UNKNOWN",reason:"KILL_SWITCH",applied:false,observedAt:new Date(now+1000).toISOString()})).toThrow(/identity\/revision mismatch/i);
    store.close();
  });
});
