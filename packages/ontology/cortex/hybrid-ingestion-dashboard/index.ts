...
export class Cortex18Error extends Error {
  constructor(public readonly code: "INVALID_INPUT" | "CONFLICT" | "PROVIDER_ERROR" | "GRAPHQL_ERROR" | "INTEGRITY_FAILURE", message: string) {
    super(message);
...
function money(value: unknown, label: string): number {
  const parsed = finite(value, label, 0, 1e13);
  const cents = Math.round(parsed * 100);
  if (!Number.isSafeInteger(cents) || Math.abs(parsed * 100 - cents) > 1e-7) throw new Cortex18Error("INVALID_INPUT", `${label} must use at most two decimal places`);
  return cents / 100;
}

function cents(value: number): number {
  const result = Math.round(value * 100);
  if (!Number.isSafeInteger(result)) throw new Cortex18Error("INVALID_INPUT", "money amount exceeds safe cent precision");
  return result;
}
...
function parseMetric(value: unknown): FinancialMetricInput {
  ...
  return Object.freeze({ source: raw.source, eventId: raw.eventId, occurredAt: utc(raw.occurredAt), currency: raw.currency, revenue: money(raw.revenue, "revenue"), cost: money(raw.cost, "cost"), spend: money(raw.spend, "spend"), conversions });
}
...
async function readBoundedJson(response: Response): Promise<unknown> {
  ...
  try {
    while (true) {
      ...
      if (total > MAX_PROVIDER_RESPONSE_BYTES) { await reader.cancel().catch(() => undefined); throw new Cortex18Error("PROVIDER_ERROR", "external metric response is oversized"); }
      chunks.push(value);
    }
...
export class HttpIncrementalMetricSource implements ExternalMetricSource {
  constructor(public readonly sourceId: string, private readonly endpoint: URL, private readonly bearerToken: string, private readonly timeoutMs = 10_000) {
    if (!ID.test(sourceId) || endpoint.protocol !== "https:" || endpoint.username || endpoint.password || endpoint.hash || endpoint.search || !bearerToken || bearerToken.length < 32 || bearerToken.length > 8_192 || /[\r\n\0]/u.test(bearerToken) || !Number.isSafeInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 60_000) throw new Cortex18Error("INVALID_INPUT", "external metric source configuration is invalid");
  }
  async poll(cursor: string | null): Promise<ExternalMetricPage> {
    ...
    const url = new URL(this.endpoint); if (cursor !== null) url.searchParams.set("cursor", cursor);
    ...
    if (new Set(items.map((item) => item.eventId)).size !== items.length) throw new Cortex18Error("PROVIDER_ERROR", "external source returned duplicate eventIds in one page");
...
export class HybridFinancialMetricStore {
  ...
  constructor(databasePath: string, private readonly now: () => number = Date.now) {
    ...
    this.db.exec(`CREATE TABLE IF NOT EXISTS cortex18_metrics(
      ...
      revenue_cents INTEGER NOT NULL,cost_cents INTEGER NOT NULL,spend_cents INTEGER NOT NULL,conversions INTEGER NOT NULL,digest TEXT NOT NULL,
  ...
  private ingestParsed(metric: FinancialMetricInput): boolean {
    ...
    this.db.prepare("INSERT INTO cortex18_metrics(source,event_id,occurred_at,currency,revenue_cents,cost_cents,spend_cents,conversions,digest) VALUES(?,?,?,?,?,?,?,?,?)").run(metric.source, metric.eventId, metric.occurredAt, metric.currency, cents(metric.revenue), cents(metric.cost), cents(metric.spend), metric.conversions, metricDigest);
    return true;
  }

  ingest(value: unknown, beforeCommit?: () => void): boolean {
    if (beforeCommit !== undefined && typeof beforeCommit !== "function") throw new Cortex18Error("INVALID_INPUT", "beforeCommit guard is invalid");
    const metric = parseMetric(value);
    this.db.exec("BEGIN IMMEDIATE");
    try {
      beforeCommit?.();
      const inserted = this.ingestParsed(metric);
      this.db.exec("COMMIT"); return inserted;
    } catch (error) { if (this.db.isTransaction) this.db.exec("ROLLBACK"); throw error; }
  }

  ingestOwnStream(stream: DurableMetricEventReader, streamName: string, consumerId: string, limit = 500, beforeCommit?: () => void): { consumed: number; inserted: number; offset: number } {
    if (!ID.test(streamName) || !ID.test(consumerId) || !Number.isSafeInteger(limit) || limit < 1 || limit > 1_000 || (beforeCommit !== undefined && typeof beforeCommit !== "function")) throw new Cortex18Error("INVALID_INPUT", "durable stream ingestion request is invalid");
    const after = stream.readOffset(consumerId, streamName); const events = stream.read(streamName, after, limit);
    ...
    for (const event of events) {
      ...
      if (this.ingest(metric, beforeCommit)) inserted += 1;
      stream.commitOffset(consumerId, streamName, event.sequence); offset = event.sequence;
    }
    return Object.freeze({ consumed: events.length, inserted, offset });
  }

  async pollExternal(source: ExternalMetricSource, beforeCommit?: () => void): Promise<{ received: number; inserted: number; nextCursor: string | null }> {
    if (!source || typeof source !== "object" || !ID.test(source.sourceId) || typeof source.poll !== "function" || (beforeCommit !== undefined && typeof beforeCommit !== "function")) throw new Cortex18Error("INVALID_INPUT", "external source is invalid");
    const initialCursor = this.sourceHealth(source.sourceId).cursor;
    const page = await source.poll(initialCursor);
    let inserted = 0;
    this.db.exec("BEGIN IMMEDIATE");
    try {
      const currentCursor = this.sourceHealth(source.sourceId).cursor;
      if (currentCursor !== initialCursor) throw new Cortex18Error("CONFLICT", "external source cursor changed during poll");
      beforeCommit?.();
      for (const item of page.items) if (this.ingestParsed(item)) inserted += 1;
      const nowMs = this.now(); if (!Number.isFinite(nowMs)) throw new Cortex18Error("INTEGRITY_FAILURE", "clock returned an invalid value");
      const lastPolledAt = new Date(nowMs).toISOString();
  ...
  summaries(): readonly FinancialSummary[] {
    const rows = this.db.prepare("SELECT currency,SUM(revenue_cents) revenue_cents,SUM(cost_cents) cost_cents,SUM(spend_cents) spend_cents,SUM(conversions) conversions,COUNT(*) events FROM cortex18_metrics GROUP BY currency ORDER BY currency").all() as Record<string, unknown>[];
    return Object.freeze(rows.map((row) => {
      const revenueCents = Number(row.revenue_cents); const costCents = Number(row.cost_cents); const spendCents = Number(row.spend_cents); const conversions = Number(row.conversions); const events = Number(row.events);
      if (![revenueCents, costCents, spendCents, conversions, events].every(Number.isSafeInteger) || Math.min(revenueCents, costCents, spendCents, conversions, events) < 0) throw new Cortex18Error("INTEGRITY_FAILURE", "stored financial summary is invalid");
      return Object.freeze({ currency: String(row.currency), revenue: fromCents(revenueCents), cost: fromCents(costCents), spend: fromCents(spendCents), profit: fromCents(revenueCents - costCents - spendCents), conversions, events });
    }));
  }
  ...
  sourceHealth(source: string): SourceHealth {
    ...
    if (!row) return Object.freeze({ source, cursor: null, lastPolledAt: null, status: "NEVER_POLLED" });
    const cursor = row.cursor === null ? null : String(row.cursor); const lastPolledAt = String(row.last_polled_at ?? ""); const date = new Date(lastPolledAt);
    if ((cursor !== null && (cursor.length < 1 || cursor.length > 1_024 || /[\r\n\0]/u.test(cursor))) || !Number.isFinite(date.getTime()) || date.toISOString() !== lastPolledAt) throw new Cortex18Error("INTEGRITY_FAILURE", "stored source health is invalid");
    return Object.freeze({ source, cursor, lastPolledAt, status: "OK" });
  }
