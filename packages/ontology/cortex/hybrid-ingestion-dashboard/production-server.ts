...
function fail(response: ServerResponse, error: unknown): void {
  ...
  if (error instanceof Cortex18Error) {
    const status = error.code === "CONFLICT" ? 409 : error.code === "PROVIDER_ERROR" ? 502 : 400;
...
function guardedSource(source: ExternalMetricSource, readMode: () => Cortex18Mode): ExternalMetricSource {
  return {
    sourceId: source.sourceId,
    async poll(cursor) {
      const page = await source.poll(cursor);
      if (mode(readMode) !== "ACTIVE") throw new ControlTransition();
      return page;
    },
  };
}
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "POST" && url.pathname === "/v1/metrics") {
        ...
        if (mode(config.readMode) !== "ACTIVE") return send(response, 503, { error: "KILLED" });
        const inserted = config.store.ingest(metric);
      ...
      if (request.method === "POST" && pollMatch) {
        ...
        const result = await config.store.pollExternal(guardedSource(source, config.readMode));
