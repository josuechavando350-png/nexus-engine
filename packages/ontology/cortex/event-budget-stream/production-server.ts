...
async function readJson(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of request) {
    const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk as Uint8Array); total += buffer.length;
    if (total > MAX_BODY_BYTES) { request.destroy(); throw new Cortex17Error("INVALID_INPUT", "request body is too large"); }
    chunks.push(buffer);
  }
...
function activeGuard(readMode: () => Cortex17Mode): () => void {
  return () => { if (failClosedMode(readMode) !== "ACTIVE") throw new ControlTransition(); };
}
...
function errorResponse(response: ServerResponse, error: unknown): void {
  ...
  if (error instanceof Cortex17Error) {
    const status = error.code === "CONFLICT" || error.code === "OFFSET_REGRESSION" ? 409 : error.code === "NOT_FOUND" ? 404 : error.code === "INTEGRITY_FAILURE" ? 500 : 400;
...
export function startCortex17Server(config: Cortex17ServerConfig): { close(): Promise<void> } {
  ...
  const writeToken = validateToken(config.writeToken, "CORTEX #17 write token"); const readToken = validateToken(config.readToken, "CORTEX #17 read token");
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/healthz") { const currentMode = failClosedMode(config.readMode); return send(response, currentMode === "KILLED" ? 503 : 200, { mode: currentMode }); }
      ...
      if (request.method === "POST" && url.pathname === "/v1/events") {
        ...
        const event = stream.append(await readJson(request), activeGuard(config.readMode));
      ...
      if (request.method === "GET" && url.pathname === "/v1/events") {
        ...
        const streamName = url.searchParams.get("stream") ?? ""; const afterRaw = url.searchParams.get("after") ?? "0"; const limitRaw = url.searchParams.get("limit") ?? "100";
      ...
      if (request.method === "POST" && url.pathname === "/v1/offsets") {
        ...
        const sequence = stream.commitOffset(body.consumerId, body.stream, body.sequence, activeGuard(config.readMode));
        return send(response, 200, { sequence });
      }

      if (request.method === "POST" && url.pathname === "/v1/budget/arbitrate") {
        if (!authorized(request, writeToken)) return send(response, 401, { error: "UNAUTHORIZED" });
        const initialMode = failClosedMode(config.readMode); if (initialMode === "KILLED") return send(response, 503, { error: "KILLED" });
        const arbitration = parseArbitrationRequest(await readJson(request)); const result = arbitrateBudget(arbitration.input);
        if (initialMode === "OBSERVE_ONLY") return send(response, 200, { mode: "OBSERVE_ONLY", result, persistedSequence: null });
        const event = stream.append({ stream: "budget.arbitration", eventId: `budget-${arbitration.decisionId}`, occurredAt: arbitration.occurredAt, payload: result }, activeGuard(config.readMode));
  ...
  let closed = false;
  return { close: () => new Promise<void>((resolve, reject) => { if (closed) return resolve(); closed = true; server.close((error) => { stream.close(); if (error) reject(error); else resolve(); }); }) };
}
