...
describe("JsonFileProgrammaticSeoPublisher", () => {
  ...
  it("enforces CAS and foreign locks, restores exact pointers, and detects artifact corruption", async () => {
    ...
    try {
      ...
      const recoveredRemoval = await publisher.apply({ kind: "REPLACE_BUNDLE", siteId: "site-a", expected: first.snapshot, desired: null }); expect(recoveredRemoval.recoveredAlreadyApplied).toBe(true); expect(recoveredRemoval.snapshot).toBeNull();
