...
import { createHyperlocalLongTailPolicy, HyperlocalLongTailCatalogProvider } from "./hyperlocal-long-tail";
...
describe("CORTEX #25 hyperlocal long-tail", () => {
  it("rejects overlapping evidence namespaces so one reference cannot satisfy geo and demand proof", () => {
    expect(() => createHyperlocalLongTailPolicy({
      version: 1,
      maxHyperlocalCatalogAgeMs: 3_600_000,
      maxHyperlocalPages: 100,
      minimumHyperlocalDistinctiveStatements: 2,
      ...
      minimumDemandEvidenceRefs: 2,
      geoEvidencePrefix: "geo",
      demandEvidencePrefix: "geo:",
      allowedHyperlocalSourceIds: ["hyperlocal-editorial-v1"],
    })).toThrowError(/non-overlapping/u);
  });
