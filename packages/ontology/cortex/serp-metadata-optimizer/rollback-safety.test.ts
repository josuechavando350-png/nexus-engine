...
class AmbiguousPublisher implements MetadataPublisher {
  calls = 0;
  failNext = false;
  constructor(private readonly delegate: JsonFileMetadataPublisher) {}
  async read(siteUrl: string, pageId: string, pageUrl: string): Promise<PublishedMetadataSnapshot | null> { return this.delegate.read(siteUrl, pageId, pageUrl); }
  async apply(action: MetadataPublishAction): Promise<MetadataPublishReceipt> {
    ...
    return this.delegate.apply(action);
  }
}
...
function harness() {
  ...
  const directory = mkdtempSync(join(tmpdir(), "nexus-serp-rollback-"));
  directories.push(directory);
  const publisher = new AmbiguousPublisher(new JsonFileMetadataPublisher({ manifestPath: join(directory, "metadata.json") }));
