...
describe("H-06 executable schema write contracts", () => {
  ...
  it("enforces unique properties inside one object type without bleeding across types", () => {
    const active = schema();
    const store = new InMemoryOntologyTransactionStore();
  ...
  it("preserves schema-enforced state through H-04 checkpoints and restore", () => {
    const active = schema();
    const store = new InMemoryOntologyTransactionStore();
    create(store, active, "c-1", { "prop.name": "Ada", "prop.meta": { nested: [1, 2] } });
    const checkpoint = store.checkpoint();
    store.transact(active.scope, active, [{ kind: "UPDATE_OBJECT", id: "c-1", expectedRevision: 1, properties: { "prop.name": "Grace" } }]);
    store.restore(checkpoint);
    expect(store.getObject(active.scope, "c-1")?.properties["prop.name"]).toBe("Ada");
    expect(store.getObject(active.scope, "c-1")?.properties["prop.meta"]).toEqual({ nested: [1, 2] });
    expect(store.getObject(active.scope, "c-1")?.revision).toBe(1);
  });
});
