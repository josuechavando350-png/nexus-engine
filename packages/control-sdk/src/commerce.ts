...
function canonicalTimestamp(value: string, field: string): string {
  ...
  if (!Number.isFinite(parsed.getTime()) || parsed.toISOString() !== value) {
    throw new CommerceControlError("INVALID_INPUT", `${field} must be canonical ISO-8601 UTC`);
  }
  return value;
}
...
function identifier(value: string, field: string, max = 200): string {
  ...
  if (!normalized || normalized.length > max || !/^[\p{L}\p{N}._:@/-]+$/u.test(normalized)) {
    throw new CommerceControlError("INVALID_INPUT", `${field} is invalid`);
  }
  return normalized;
}
...
function validatePrincipal(principal: CommercePrincipal, tenantId: string, permission: CommercePermission): CommercePrincipal {
  ...
  const principalId = identifier(principal.principalId, "principalId");
  const principalTenant = identifier(principal.tenantId, "principal.tenantId");
  if (principalTenant !== tenantId || !principal.permissions.includes(permission)) {
    throw new CommerceControlError("FORBIDDEN", `principal is not authorized for ${permission} in tenant ${tenantId}`);
  }
  return { ...principal, principalId, tenantId: principalTenant };
}
...
function validateRequest(input: CommerceActionRequest): Omit<CommerceActionRequest, "scope"> & { scope: CommerceScope } {
  ...
  if (!/^[a-f0-9]{64}$/.test(input.payloadDigest)) throw new CommerceControlError("INVALID_INPUT", "payloadDigest must be a lowercase SHA-256 digest");
  return Object.freeze({
...
export class GovernedCommerceEngine {
  private readonly transactions = new Map<string, CommerceTransaction>();
  private readonly idempotency = new Map<string, { actionDigest: string; transactionId: string }>();
  private readonly auditEvents: CommerceAuditEvent[] = [];
  private readonly inFlight = new Map<string, Promise<CommerceTransaction>>();
  ...
  constructor(
    ...
    if (!Number.isSafeInteger(executionTimeoutMs) || executionTimeoutMs < 1 || executionTimeoutMs > 120_000) {
      throw new CommerceControlError("INVALID_INPUT", "execution timeout must be between 1 and 120000ms");
    }
  }
  ...
  prepare(principal: CommercePrincipal, requestInput: CommerceActionRequest, nowInput: string): CommerceTransaction {
    ...
    try {
      validatePrincipal(principal, request.scope.tenantId, "commerce:prepare");
    } catch (error) {
    ...
    if (existing) {
      ...
      return this.requireTransaction(request.scope.tenantId, existing.transactionId);
    }
  ...
  decideApproval(
    ...
    try {
      validatePrincipal(principal, tenantId, "commerce:approve");
    } catch (error) {
      this.auditDenied(principal, tenantId, now, error instanceof Error ? error.message : "authorization denied", transactionId);
      throw error;
    }
    const current = this.requireTransaction(tenantId, transactionId);
    ...
    const approval = Object.freeze({
      approvalId: `commerce_approval_${digest(core)}` as const,
      ...core,
      approvalDigest: digest(core),
    });
  ...
  execute(principal: CommercePrincipal, tenantIdInput: string, transactionId: string, nowInput: string, signal?: AbortSignal): Promise<CommerceTransaction> {
    const tenantId = identifier(tenantIdInput, "tenantId");
    const now = canonicalTimestamp(nowInput, "now");
    try {
      validatePrincipal(principal, tenantId, "commerce:execute");
    } catch (error) {
      this.auditDenied(principal, tenantId, now, error instanceof Error ? error.message : "authorization denied", transactionId);
  ...
  getTransaction(principal: CommercePrincipal, tenantIdInput: string, transactionId: string): CommerceTransaction {
    const tenantId = identifier(tenantIdInput, "tenantId");
    validatePrincipal(principal, tenantId, "commerce:read");
    return this.requireTransaction(tenantId, transactionId);
  }

  listAuditEvents(principal: CommercePrincipal, tenantIdInput: string): readonly CommerceAuditEvent[] {
    const tenantId = identifier(tenantIdInput, "tenantId");
    validatePrincipal(principal, tenantId, "commerce:read");
    return Object.freeze(this.auditEvents.filter((event) => event.tenantId === tenantId).map((event) => Object.freeze({ ...event })));
  }
  ...
  private async executeOnce(principal: CommercePrincipal, current: CommerceTransaction, now: string, signal?: AbortSignal): Promise<CommerceTransaction> {
    if (current.state !== "APPROVED" || !current.approval || current.approval.decision !== "GRANTED") {
      throw new CommerceControlError("APPROVAL_REQUIRED", "exact action approval is required before commerce execution");
    }
    if (!sameScope(current.scope, current.approval.scope) || current.approval.actionDigest !== current.actionDigest) {
      throw new CommerceControlError("APPROVAL_REQUIRED", "approval is not bound to this exact scoped action");
    }
    if (new Date(current.approval.expiresAt).getTime() <= new Date(now).getTime()) {
      throw new CommerceControlError("APPROVAL_EXPIRED", "commerce approval expired before execution");
    }
    ...
    const onAbort = () => controller.abort(signal?.reason ?? new Error("commerce execution cancelled"));
    signal?.addEventListener("abort", onAbort, { once: true });
    const timer = setTimeout(() => controller.abort(new Error("commerce execution timeout")), this.executionTimeoutMs);
    try {
      const result = await this.executor.execute(Object.freeze({
        ...
        actionDigest: current.actionDigest,
        approvalDigest: current.approval.approvalDigest,
      }), controller.signal);
  ...
  private audit(principalId: string, tenantId: string, occurredAt: string, action: CommerceAuditEvent["action"], detail: string, transactionId?: string): void {
    ...
    const core = {
      eventId: `commerce_audit_${digest({ tenantId, transactionId: transactionId ?? null, principalId, action, occurredAt, detail, previousDigest, sequence: tenantEvents.length })}` as const,
      tenantId,
      transactionId,
      principalId,
      action,
...
export class GovernedCommerceRuntime {
  ...
  prepare(principal: CommercePrincipal, request: CommerceActionRequest, now: string): CommerceTransaction {
    return this.engine.prepare(principal, request, now);
  }

  approve(principal: CommercePrincipal, tenantId: string, transactionId: string, now: string, expiresAt: string): CommerceTransaction {
    return this.engine.decideApproval(principal, tenantId, transactionId, "GRANTED", now, expiresAt);
  }

  deny(principal: CommercePrincipal, tenantId: string, transactionId: string, now: string, expiresAt: string): CommerceTransaction {
    return this.engine.decideApproval(principal, tenantId, transactionId, "DENIED", now, expiresAt);
  }

  execute(principal: CommercePrincipal, tenantId: string, transactionId: string, now: string, signal?: AbortSignal): Promise<CommerceTransaction> {
    return this.engine.execute(principal, tenantId, transactionId, now, signal);
  }
}
